...
buildBookshelf = function(options){
  var currentDemo,
    lastDemo,
    previewDemo,
    previewNumPages,
    ...
    //   'sam-test' : {'path' : 'samples/sam-test/'},
  ...
  bookshelf = {
    // attaches the callback to the sample
    // (usage: callback should initialise this flipbook instance)
    ...
    open: function(demoName) {
      ...
      if (sample.callback) {
        ...
        sample.callback(); // initialise this flipbook instance

      } else {
        // couldn't find an initialise callback, which is run as soon as 'demoName'.js is loaded
        // hence, load .js/.css before tring again
        ...
        //   ]
        // });
        zine(demoName, sample.path, sample.pages);
    ...
    // load any supplied pictures before performing callback
    ...
    var test = [],
      ...
      loadNextTest = function() {
        ...
        if (sample.loading==status.loading) {
          ...
          //     if (sample.callback)
          //       // just seems to load html4 if necessary
  ...
  // onload = function(){
    ...
    clickElement($('.bookshelf'), function(e) {
      if (e.target && $(e.target).hasClass('sample')) {
        Hash.go('samples/' + $(e.target).attr('sample')); // sample is attribute in index.html (e.g steve-jobs)
      }
    });
