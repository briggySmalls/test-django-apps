...
// handy object storing state class lablels
var states = {};
...
function positionZines(){
	...
	$(containerSelector).each(function(){
		...
		var applyCalcs = function(){
			...
			if (container.hasClass(states.ACTIVATED)){
		...
		// TO DO: check if this test can handle multiple classes
		if (container.hasClass(states.DEACTIVATING) || container.hasClass(states.ACTIVATING) || container.hasClass(states.ZOOMING)){
			// find transition namespace
			var namespace;
			if (container.hasClass(states.DEACTIVATING)){
				namespace = states.DEACTIVATING;
			} else if (container.hasClass(states.ACTIVATING)){
				namespace = states.ACTIVATING;
			} else if (container.hasClass(states.ZOOMING)){
				namespace = states.ZOOMING;
			}
...
function moveZine(container,state) {
	...
	switch (state){
		...
		container.removeClass(states.DEACTIVATED);
		// indicate zine is not zoomed (if it was)
		// TO DO: make this a small helper function
		container.removeClass(states.ZOOMED);
		container.off('drag');
		container.off('dragend');

		// INDICATE INTERMEDIATE STATUS
		container.addClass(states.ACTIVATING);

		// SET STATE FOR UPCOMING STATUS
		transitionEventTag = transitionEndName(states.ACTIVATING);
		container.on(transitionEventTag,function(){
			...
			// signal status
			container.removeClass(states.ACTIVATING).addClass(states.ACTIVATING);
			// setup next click behaviour
			container.one('click',function(){
				// if clicked in active state, zoom
				moveZine(container,states.ZOOMED);
			});
		...
		// INDICATE INTERMEDIATE STATUS
		container.addClass(states.ZOOMING);
